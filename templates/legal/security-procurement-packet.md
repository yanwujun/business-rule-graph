...
| **Findings registry** | `.roam/findings.sqlite` | Normalised cross-detector finding table (29 detectors are wired for findings-registry persistence as of v13.2 — clones, dead, complexity, smells, n1, missing-index, over-fetch, bus-factor, auth-gaps, vulns, invariants, hotspots, taint, vibe-check, orphan-imports, conventions, pr-risk, duplicates, audit-trail-conformance/-verify, boundary, test-hermeticity, plus aggregators; the registry stores last-run state per detector, not a cumulative tally). | `roam findings list / show / count` for per-detector totals; `<DETECTOR>_DETECTOR_VERSION` stamps for drift tracking. |
