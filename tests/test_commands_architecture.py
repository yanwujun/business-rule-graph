...
# W414: All architecture-command tests in this file are read-only against
# the indexed project (map/layers/clusters/coupling/entry-points/patterns/
# safe-zones/visualize). Re-indexing on every test costs ~2s x 63. Override
# at module scope here so the project is built and indexed once per worker.
# Mirrors W346's pattern in test_json_contracts.
@pytest.fixture(scope="module")
def indexed_project(tmp_path_factory):
    """Module-scoped indexed Python project for read-only architecture tests."""
    import textwrap

    proj = tmp_path_factory.mktemp("commands_architecture_proj")
