...
def test_vulns_empty_corpus_distinguishes_no_scan_from_no_findings(
    ...
    assert result.exit_code == 0, (
        f"`roam --json vulns` on empty corpus exited "
        f"{result.exit_code}, expected 0.\n--- stdout ---\n{result.output}"
    )

    # 2. Structured envelope.
    env = _json.loads(result.output)
    assert env.get("command") == "vulns", (
        f"Expected envelope.command == 'vulns', got {env.get('command')!r}"
    )
    ...
    assert any(
        token in verdict_lc
        for token in ("no vulnerability scan", "no scan", "empty", "import-file")
    ), (
    ...
    assert isinstance(facts, list) and facts, (
        f"agent_contract.facts must be a non-empty list on the empty-corpus "
        f"path. Got contract={contract!r}"
    )
    # Each fact should be a non-empty string.
    for i, f in enumerate(facts):
        assert isinstance(f, str) and f.strip(), (
            f"agent_contract.facts[{i}] must be a non-empty string. "
            f"Got facts={facts!r}"
        )
...
def test_vulns_empty_corpus_total_is_zero(tmp_path, monkeypatch):
    ...
    assert all(v == 0 for v in by_sev.values()), (
        f"Empty corpus must not report any severity counts. Got {by_sev!r}"
    )
    ...
    assert isinstance(vulns, list) and len(vulns) == 0, (
        f"Empty corpus must emit `vulnerabilities: []`. Got {vulns!r}"
    )
