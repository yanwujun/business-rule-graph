...
class TestRegistry:
    ...
    def test_tasks_tagged_in_shared_catalog(self):
        ...
        assert set(ids) == {"tool-description-declarative", "weak-verdict", "missing-next-command"}
        for tid in ids:
...
class TestCli:
    def test_list_tasks_json(self):
        ...
        assert env["summary"]["task_count"] == 3
    ...
    def test_list_detectors_json(self):
        ...
        assert r.exit_code == 0, r.output
        env = _parse_env(r.output)
        assert env["summary"]["detector_count"] == 3
