"""W1085 — Pattern-1A disclosure on cmd_fingerprint hard-cap refusal.

The pre-W1085 envelope on the `sym_count > _HARD_CAP_SYMBOLS` branch had:

    summary = {
        "verdict": "Graph too large (...) for fingerprint analysis. ...",
        "symbol_count": ...,
        "hard_cap": ...,
    }
...
W1085 (Pattern-1A canonical fix) adds:
...
The test forces the over-cap path by monkeypatching `_HARD_CAP_SYMBOLS` down
to 0 (so any non-empty index trips the branch). Synthesizing a > 100k symbol
corpus on disk would be prohibitively expensive.
...
def _make_minimal_corpus(tmp: Path) -> None:
    ...
    The corpus has 1+ symbols so the over-cap branch (rather than the W805
    `sym_count == 0` empty-corpus branch) is the one we exercise.
    """
...
class TestFingerprintHardCapDisclosure:
    """W1085 — over-cap refusal must surface partial_success + state."""
    ...
    def test_over_cap_envelope_discloses_graph_too_large(self, tmp_path, monkeypatch):
        ...
        # the branch.
        from roam.commands import cmd_fingerprint as fp_mod

        monkeypatch.setattr(fp_mod, "_HARD_CAP_SYMBOLS", 0)
    ...
    def test_under_cap_normal_run_unaffected(self, tmp_path, monkeypatch):
        ...
        With the default cap (100_000), a 2-symbol fixture must take the
        normal spectral-analysis path and emit the regular topology
        envelope — NOT the W1085 graph_too_large branch.
        """
        _make_minimal_corpus(tmp_path)
        ...
        # Normal path: state must NOT be the W1085 disclosure.
        assert summary.get("state") != "graph_too_large", (
            f"under-cap run should not enter W1085 disclosure; summary={summary!r}"
        )
