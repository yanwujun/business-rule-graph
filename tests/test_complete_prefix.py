...
class TestCompletePrefix:
    def test_prefix_matches_use_prefixed_symbols(
        self, cli_runner, prefix_project, monkeypatch
    ):
        """``roam complete use`` returns ``useFoo`` and ``useBar``."""
        monkeypatch.chdir(prefix_project)
        result = invoke_cli(
            cli_runner, ["complete", "use"], cwd=prefix_project, json_mode=True
        )
        data = parse_json_output(result, "complete")
        symbols = _symbol_completions(data)
        assert "useFoo" in symbols, (
            f"prefix `use` should match `useFoo`; got symbols: {symbols}"
        )
        assert "useBar" in symbols, (
            f"prefix `use` should match `useBar`; got symbols: {symbols}"
        )

    def test_prefix_does_not_match_internal_substring(
        self, cli_runner, prefix_project, monkeypatch
    ):
        ...
        assert "MyUseFoo" not in symbols, (
            "prefix `use` should NOT match `MyUseFoo` (substring, not "
            f"prefix); got symbols: {symbols}"
        )

    def test_verdict_declares_prefix_mode(
        self, cli_runner, prefix_project, monkeypatch
    ):
        ...
        assert summary.get("match_mode") == "prefix", (
            f"expected summary.match_mode == 'prefix', got {summary!r}"
        )
