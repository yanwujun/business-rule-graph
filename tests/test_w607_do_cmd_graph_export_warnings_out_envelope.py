...
def test_w978_7_discipline_ast_audit_do():
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        func = node.func
        if not isinstance(func, ast.Name) or func.id != "_run_check_do":
            continue
        if not node.args:
            continue
        phase_arg = node.args[0]
        assert isinstance(phase_arg, ast.Constant), (
            f"_run_check_do phase arg must be a string literal at line {phase_arg.lineno}; got {ast.dump(phase_arg)!r}"
        )
            ...
            if isinstance(value, (ast.List, ast.Dict, ast.Tuple, ast.Set)):
                ...
                for child in children:
                    assert _is_literal_tree(child), (
                        f"_run_check_do default= contains non-literal child at line {value.lineno}: {ast.dump(child)!r}"
                    )
                continue
            if isinstance(value, ast.Name):
                assert value.id in ("None", "True", "False"), (
                    f"_run_check_do default= references symbol {value.id!r} "
                    f"at line {value.lineno}; only literals + immutable "
                    f"containers allowed (W978 2nd discipline)."
                )
                continue
            raise AssertionError(f"_run_check_do default= is not a literal at line {value.lineno}: {ast.dump(value)!r}")
