...
def _build_summary(
    ...
    if cli_total and mcp_total:
        cap_phrase = (
            f" {cli_total} CLI commands and {mcp_total} MCP tools are available;"
            f" run `roam --help-all` to list them."
        )
    elif cli_total:
...
def generate_agents_md(
    ...
    *,
    ...
    if am.danger_zones:
        sources_consulted["danger_zones"] = (
    ...
    if with_constitution:
        ...
        if pre or after or prep:
            sources_consulted["gates"] = (
                "roam.constitution.loader" + ("" if path else " (defaults)")
            )
...
def _render_stack(stack: list[dict[str, Any]]) -> list[str]:
    ...
    for entry in stack:
        lines.append(
            f"- **{entry['language']}** ({entry['files']} files, {entry['pct']}%)"
        )
    lines.append("")
    return lines
...
def _render_danger(zones: list[dict[str, Any]]) -> list[str]:
    ...
    lines.append(
        "Top files by `churn x complexity x max_fan_in`. "
        "Edit these only after running `roam preflight <symbol>`:"
    )
    lines.append("")
    ...
    for z in zones:
        lines.append(
            f"| `{z['path']}` | {z['danger_score']} | {z['churn']} | "
            f"{z['complexity']} | {z['max_fan_in']} |"
        )
    lines.append("")
    lines.append(
...
def _render_current_mode(mode: dict[str, Any]) -> list[str]:
    ...
    lines = ["## Current mode", ""]
    lines.append(
        f"Active mode: **{name}** ({allowed_count} command(s) allowed)."
    )
    if valid_modes:
        joined = " | ".join(valid_modes)
        lines.append(
            f"Switch with: `roam mode <{joined}>`"
        )
    lines.append("")
    ...
    if blocked and upgrade_to:
        lines.append(
            f"**Blocked in `{name}` mode** (unlock with `roam mode {upgrade_to}`):"
        )
        lines.append("")
        ...
        lines.append(
            "_This is the highest mode -- every documented command is allowed._"
        )
        lines.append("")
...
def render_markdown(am: AgentsMd) -> str:
    ...
    lines.append("")
    lines.append(
        f"> Auto-generated by `roam agents-md` at {am.generated_at}."
        f" Run `roam agents-md --refresh` to update."
    )
    lines.append("")
