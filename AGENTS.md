...
a local SQLite DB. **277 commands · 244 MCP tools (16 in the default `core` preset) · 28 languages · 100% local · zero API keys.**
<!-- END auto-count:Codex-headline -->

<!-- BEGIN auto-count:Codex-authoritative -->
Authoritative counts (AST-derived, env-independent): `command_count: 277 · canonical_count: 270 · category_count: 7 · mcp tools registered: 244 · mcp tools in core preset: 16`. The `roam surface --json` envelope additionally exposes `mcp_tool_count_by_preset` for per-preset counts.
...
```
src/roam/
  cli.py              # Click CLI entry point — LazyGroup, _COMMANDS dict, _CATEGORIES. 277 command names (270 canonical + 7 aliases).
...
Run `roam --help` for the 5-verb core; `roam --help-all` for all 277 command names; `roam surface --json` for the machine-readable inventory. Use `roam --json <cmd>` for structured output.
