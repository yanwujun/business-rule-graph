...
- **Hardened local telemetry and replay containment** — compile telemetry is explicit opt-in, aggregate-only, size/time bounded, and owner-only; prompt/session/turn/episode fingerprints are excluded. Sibling-patch replay uses literal argv, a scrubbed environment, isolated HOME/TMP roots, bounded process-tree termination, and explicitly discloses that this is process containment rather than a kernel sandbox.
...
- **Claude hook installation is a bounded control-plane update** — settings and hook bodies are size-bounded, strict-JSON parsed, protected from duplicate keys, symlinks, junctions, hard links, and concurrent replacement, then written through the canonical durable compare-and-swap atomic writer with exact-interpreter commands. Unsafe or changed paths stop without clobbering user content; recognized prior bodies remain healable from a closed digest registry.
...
- **Historical transcript mining now fails closed around private filesystem state** — `.roam`, the keyed fingerprint salt, source JSONL files, and the derived snapshot are bounded and checked without following symlinks, junctions, or hard links. Keys are created owner-only with exclusive first-writer semantics and bounded concurrent-read retries; snapshots use unpredictable same-directory temporaries, compare the destination immediately before replacement, and durably sync both file contents and the directory entry. Unsafe paths produce a structured error envelope and never touch the redirected target.
