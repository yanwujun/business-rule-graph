...
<sub>275 commands · 244 MCP tools (16 in the default `core` preset) · 28 languages</sub>
...
<summary><strong>Full command reference — canonical command list (all 268)</strong></summary>
...
| `roam_ask` | Natural-language codebase question dispatcher. Examples: 'is it safe to delete X?', 'where does login validate?', 'what just broke?', 'who owns module Y?'. Routes intent to one recipe in the graph-aware 30-recipe registry. One call replaces Grep+Read for most questions. Run this FIRST when the user asks a code-comprehension question. |
